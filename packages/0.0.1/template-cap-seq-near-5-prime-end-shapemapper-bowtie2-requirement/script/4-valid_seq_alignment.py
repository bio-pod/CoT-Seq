...
for name, R1R2_dir, R1, R2 in R1R2s:
    ...
    in_sam = f'{bowtie_dir}/{R2}-without-cap.sam'
    ...
    print(f'valid_num: {valid_num} ({valid_num/num_of_alignments*100.0:.2f})% invalid_num: {invalid_num} ({invalid_num/num_of_alignments*100.0:.2f})% align_to_reverse_num: {align_to_reverse_strand} ({align_to_reverse_strand/num_of_alignments*100.0:.2f})%', flush=True)
    valid_out_f.close()
    invalid_out_f.close()
