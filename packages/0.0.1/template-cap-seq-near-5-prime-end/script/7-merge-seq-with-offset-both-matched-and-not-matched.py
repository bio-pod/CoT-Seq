...
for name, R1R2_dir, R1, R2 in R1R2s:
    ...
    for target_pos in range(1, len_template_seq+1):
        ...
        for offset in offsets:
            ...
            append_file_to_file(f'{match_dir}/{name}/not-matched/{pos:0{len(str(len_template_seq))}d}/{pos:0{len(str(len_template_seq))}d}-R1/{R1}', f'{out_R1_dir}/{R1}')
            append_file_to_file(f'{match_dir}/{name}/not-matched/{pos:0{len(str(len_template_seq))}d}/{pos:0{len(str(len_template_seq))}d}-R2/{R2}', f'{out_R2_dir}/{R2}')
