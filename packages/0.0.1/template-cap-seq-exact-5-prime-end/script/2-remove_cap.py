...
for name, R1R2_dir, R1, R2 in R1R2s:
    ...
    out_R2 = f'{result_dir}/{R2}-clean-without-cap'
            ...
            if R2_l[0] == '@':
                ...
                if tmp_entry_lines != None:
                    ...
                    if R2_seq[:len(cap_seq)] == cap_seq:
                        seq_num += 1
                        valid_seq = R2_seq[len(cap_seq):]
                        valid_seq_len = len(valid_seq)
                        valid_qualities = quality_values[len(cap_seq):]
                        ...
                        print(f'cap not match for {R2_seq}', flush=True)
                        exit()
                tmp_entry_lines = []
    ...
    print(f'seq_num: {seq_num}', flush=True)
