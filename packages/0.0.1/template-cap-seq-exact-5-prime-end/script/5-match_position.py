...
for name, R1R2_dir, R1, R2 in R1R2s:
    ...
    in_R1 = f'{result_dir}/{R1}-clean'
    in_R2 = f'{result_dir}/{R2}-clean'
            ...
            if R1_l[0] == '@':
                ...
                if tmp_entry_lines != None:
                    ...
                    if R2_seq[:len(cap_seq)] == cap_seq:
                        R2_end_seq = R2_seq[len(cap_seq):len(cap_seq)+match_num]
    ...
    print(f'matched_num -> {matched_num} ({matched_num/(matched_num+not_matched_num)*100.0}%)  not_matched_num -> {not_matched_num} ({not_matched_num/(matched_num+not_matched_num)*100.0}%)', flush=True)
